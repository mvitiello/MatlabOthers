function vanna = blsvanna(so,x,r,t,sig,q) 
%   V = BLSVANNA(SO,X,R,T,SIG) returns  the rate of change in the delta 
...
%   For example, try v = blsvanna(50,50,.12,.25,.3,0)
...
d2 = d1 - sig .* sqrt(t);
vanna = -exp(- q .* t)*normpdf(d1) .* d2 ./ sig;
end