function speed = blsspeed(so,x,r,t,sig,q) 
%   Vt = BLSSPEED(SO,X,R,T,SIG) measures the rate of change 
%   in Gamma with respect to changes in the underlying price.  
...
%   For example, try v = blsspeed(50,50,.12,.25,.3,0)
...
speed = ((-exp(- q .* t)*normpdf(d1))./ (sig .* so .^ 2 .* sqrt(t))).* ...
    ((d1./(sig .* sqrt(t))) + 1);
end