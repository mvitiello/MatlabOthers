function color = blscolor(so,x,r,t,sig,q) 
%   Vt = BLSCOLOR(SO,X,R,T,SIG) measures the rate of change in 
...
%   For example, try v = blscolor(50,50,.12,.25,.3,0)
...
d2 = d1 - sig .* sqrt(t);
color = (-exp(- q .* t)*normpdf(d1))./ (2 .* so .* t .* sig .* ...
    sqrt(t)).* (2 .* q .* t + 1 + d1.*((2 .* (r - q).*t - d2 .*...
    sig .* sqrt(t))./(sig .* sqrt(t))));
end